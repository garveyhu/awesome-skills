# Docsify Provider · 零构建 markdown 文档站

> 本文是 `docs-site-creator` 的 Docsify provider 手册(选型见上层 SKILL.md)。
> 适用:单项目 / 内部 / 快速可浏览的文档;可编译单文件离线 HTML。
...
This provider turns an **existing** `docs/` of markdown into a site. If the project has no docs yet (or they're thin), generate them first with the sibling skill **`wiki-creator`** (deep-scans the codebase → structured `docs/` with Mermaid), then run this provider on the result. If `wiki-creator` isn't installed, skip it and just use whatever `docs/` exists — don't hard-depend on it.

## 设计原则:克制 emoji(重要)

This provider produces a deliberately refined, "high-end" site. **Avoid emoji wherever you can** — none in the sidebar tree, group headers, page/section titles, the cover, mermaid node labels, or callout markers. Emoji read as cheap / AI-generated. Lean on the built-in gradient accents, SVG icons, and typography for hierarchy. Only keep incidental emoji already present in the source prose if the user explicitly wants them.
...
- **双击即开**:file:// 打开即可、无需本地服务器;**需联网**以从共享库源取库。
- **只请求自托管库源**:除共享库走 `cdn.archeruuu.com` 外无任何外部请求(无第三方 CDN、无网络字体)。
- 文档/动画改动后**重跑脚本**刷新。
...
Copy this provider's `assets/anim-kit/` into the generated site:
...
Theme: `anim.css` defines the CSS color variables for both `body.dark` (this provider's convention) and `html[data-theme]`, and `anim-core.js` reads them from `document.body` — so animations auto-match the site's light/dark theme. No extra wiring.
