...
| **codex-conductor** | 「Claude 当大脑、Codex 当手」的派活工作流：实现类任务经 `codex:codex-rescue` agent 派给本机 Codex CLI——Claude 拆任务、写任务书、**独立验收**（Codex 完工报告不作数）、亲自合并。**角色档位**（builder / chore / detective / scout / reviewer，每个 = 模型档 + effort + 读写 + prompt 姿态的预设组合）+ 任务书范式 + worktree 纪律 + 后台任务轮询。从一场 9 个工作包全由 Codex 实现的实战沉淀 |
