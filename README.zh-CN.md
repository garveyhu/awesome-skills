...
| skill | 做什么 |
|---|---|
| **browser-runner** | 把重复的浏览器操作沉淀成「确定性、可复放」的 Playwright-over-CDP 脚本：脚本 attach 你真实已登录的 Chrome（登录态/指纹原样·封控风险最低），一个流程一目录（`flow.toml` 元信息 + `flow.py`），加流程不碰内核；另配一个元信息驱动的轻量本地看板，自动生成参数表单、一键触发、实时输出 |
