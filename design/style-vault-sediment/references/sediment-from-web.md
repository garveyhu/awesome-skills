...
用户指出沉淀和参考站"差异大"（常见于只拉 URL 没要截图时）→ 走 [shared-workflow.md 步骤 9 · 教训回写](shared-workflow.md#步骤-9--教训回写条件触发) + [lessons-loopback.md](lessons-loopback.md)。

**from-web 最常见的"模式错"类型**：
- 单靠 `WebFetch` 返回的 HTML 文本判定视觉（颜色 / 字体 / 布局密度），没要求用户贴截图做视觉兜底
- 把在线站的源码/压缩 CSS 直接当视觉依据（版权 + 混淆双重问题）
- 截图只看 1 张 hero 就开始抽象，没覆盖发现 / 详情 / 管理后台等多路由场景
- Tier 3 from-web 不降档（源码不可得 → 全路由枚举物理不可能，实际产出量 = Tier 2）

一次小错（单条条目字段错了）→ 改条目即可，**不要**触发教训回写。
