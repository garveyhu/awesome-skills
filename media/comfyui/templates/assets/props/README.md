...
纯色底 + `keyflat` 抠透明 + `pixelize` 统一风格。量大可再分子目录(如 `decor/`、`furniture/`),每个子目录各放一份 assets.json。
...
2. 项目根预览:`bash scripts/comfyui/batch.sh --only props --dry-run`。
3. 出图:去掉 `--dry-run`(过夜在自己终端跑,`--variants N` 控制每条几张)。
4. 满意的把 item `status` 改 `done`,本表改 ✅ 补预览。

## 集成

切片/接入运行时目录(如 `frontend/public/props/`)后,状态标 📦。
