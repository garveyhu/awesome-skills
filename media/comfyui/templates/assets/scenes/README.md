...
整图,通常**不抠**(assets.json 里 keyflat/pixelize 设 null);质量要求高可用顶质量底模。
...
2. 项目根预览:`bash scripts/comfyui/batch.sh --only scenes --dry-run`。
3. 出图:去掉 `--dry-run`(过夜在自己终端跑,`--variants N` 控制每条几张)。
4. 满意的把 item `status` 改 `done`,本表改 ✅ 补预览。

## 集成

切片/接入运行时目录(如 `frontend/public/scenes/`)后,状态标 📦。
