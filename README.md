...
| **pdf-craft** | Typeset content into print-grade PDFs via HTML+CSS — four layout systems (government filing / business proposal / technical report / resume) plus a typography linter for Chinese print conventions |
...
| skill | what it does |
|---|---|
| **skill-management** | My skill-management methodology itself: organize many skills as source → category → skill, driven by a single `registry`, reusable across Claude Code, Gemini, Pi & Codex with per-agent sync whitelists and bundled tooling. **Any AI can read it and replicate the whole setup** |
...
| **codex-conductor** | "Claude as brain, Codex as hands" delegation workflow: dispatch implementation work to the local Codex CLI via the `codex:codex-rescue` agent — Claude decomposes, writes the task brief, **independently verifies** (Codex's own completion report never counts) and merges itself. **Role presets** (builder / coder / chore / detective / scout / reviewer — each a model-tier + effort + read-write + prompt-stance combo, one reference file per role), task-brief patterns, worktree discipline, background-job polling. Distilled from a real 9-work-package campaign fully implemented by Codex |
