...
| **codex-conductor** | "Claude as brain, Codex as hands" delegation workflow: dispatch implementation work to the local Codex CLI via the `codex:codex-rescue` agent — Claude decomposes, writes the task brief, **independently verifies** (Codex's own completion report never counts) and merges itself. **Role presets** (builder / chore / detective / scout / reviewer, each a model-tier + effort + read-write + prompt-stance combo), task-brief patterns, worktree discipline, background-job polling. Distilled from a real 9-work-package campaign fully implemented by Codex |
